...
new_level:
    # Example: Draw a horizontal line from (5,10) to (5,20)
    addi x7, x0, 5     # row
    addi x8, x0, 32    # scale factor
    mul x9, x7, x8     # base offset for row 5

    addi x10, x0, 10   # start column
    addi x11, x0, 20   # end column

new_level_loop:
    add x12, x9, x10   # x9 = 5*32 + col
    sw x30, 0(x12)     # store to RAM
    add x13, x3, x12
    sw x30, 0(x13)     # store to VGA

    addi x10, x10, 1
    bgt x10, x11, end_program
    jal x0, new_level_loop
